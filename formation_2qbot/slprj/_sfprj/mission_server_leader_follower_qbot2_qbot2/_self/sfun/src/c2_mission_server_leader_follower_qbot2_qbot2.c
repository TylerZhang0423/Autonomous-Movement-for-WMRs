  ...
  (SFc2_mission_server_leader_follower_qbot2_qbot2InstanceStruct *chartInstance)
{
  ...
  real_T c2_c_hoistedGlobal;
  real_T c2_d_hoistedGlobal;
  real_T c2_x_waypoint;
  real_T c2_z_waypoint;
  real_T c2_x_ugv;
  real_T c2_z_ugv;
  ...
  int32_T c2_i0;
  real_T c2_b_x[2];
  real_T *c2_b_x_waypoint;
  real_T *c2_b_x_ref;
  real_T *c2_b_z_waypoint;
  real_T *c2_b_x_ugv;
  real_T *c2_b_z_ugv;
  real_T *c2_b_z_ref;
  ...
  c2_b_z_waypoint = (real_T *)ssGetInputPortSignal(chartInstance->S, 1);
  c2_b_x_ref = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c2_b_x_waypoint = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
  _SFD_SYMBOL_SCOPE_PUSH(0U, 0U);
  _sfTime_ = sf_get_time(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 1U, chartInstance->c2_sfEvent);
  _SFD_DATA_RANGE_CHECK(*c2_b_x_waypoint, 0U);
  chartInstance->c2_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 1U, chartInstance->c2_sfEvent);
  c2_hoistedGlobal = *c2_b_x_waypoint;
  c2_b_hoistedGlobal = *c2_b_z_waypoint;
  c2_c_hoistedGlobal = *c2_b_x_ugv;
  c2_d_hoistedGlobal = *c2_b_z_ugv;
  c2_x_waypoint = c2_hoistedGlobal;
  c2_z_waypoint = c2_b_hoistedGlobal;
  c2_x_ugv = c2_c_hoistedGlobal;
  c2_z_ugv = c2_d_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c2_nargout, 3U, c2_b_sf_marshallOut,
    c2_b_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_x_waypoint, 4U, c2_b_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_z_waypoint, 5U, c2_b_sf_marshallOut);
  ...
  (real_T)_SFD_EML_ARRAY_BOUNDS_CHECK("x_waypoint", (int32_T)_SFD_INTEGER_CHECK(
    "idx", chartInstance->c2_idx), 1, 1, 1, 0);
  (real_T)_SFD_EML_ARRAY_BOUNDS_CHECK("z_waypoint", (int32_T)_SFD_INTEGER_CHECK(
    "idx", chartInstance->c2_idx), 1, 1, 1, 0);
  c2_x[0] = c2_x_ugv - c2_x_waypoint;
  c2_x[1] = c2_z_ugv - c2_z_waypoint;
  for (c2_i0 = 0; c2_i0 < 2; c2_i0++) {
    c2_b_x[c2_i0] = c2_x[c2_i0];
  }
  ...
  if (CV_EML_COND(0, 1, 0, c2_dist > c2_DIST_THRESHOLD)) {
    guard1 = true;
  } else if (CV_EML_COND(0, 1, 1, chartInstance->c2_idx == 1.0)) {
    guard1 = true;
  } else {
    ...
    (real_T)_SFD_EML_ARRAY_BOUNDS_CHECK("x_waypoint", (int32_T)
      _SFD_INTEGER_CHECK("idx", chartInstance->c2_idx), 1, 1, 1, 0);
    c2_x_ref = c2_x_waypoint;
    _SFD_EML_CALL(0U, chartInstance->c2_sfEvent, 29);
    (real_T)_SFD_EML_ARRAY_BOUNDS_CHECK("z_waypoint", (int32_T)
      _SFD_INTEGER_CHECK("idx", chartInstance->c2_idx), 1, 1, 1, 0);
    c2_z_ref = c2_z_waypoint;
  }
  ...
  if (CV_EML_IF(0, 1, 2, chartInstance->c2_idx == 1.0)) {
  ...
  *c2_b_mission_complete = c2_mission_complete;
  ...
  _SFD_DATA_RANGE_CHECK(*c2_b_z_waypoint, 2U);
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
...

        {
          ...
          c2_z_waypoint = (real_T *)ssGetInputPortSignal(chartInstance->S, 1);
          c2_x_ref = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
          c2_x_waypoint = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
