...
typedef RT_MODEL_leader_qbot2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * leader_qbot2(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void leader_qbot2_initialize();
   extern void leader_qbot2_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(leader_qbot2_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "tcpip://192.168.2.47:17001?nagle=no,keep_alive=1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: leader_qbot2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
      ...
      if (result == 0) {
        ...
        if (result == 0) {
          ...
          if (result == 0) {
            ...
            {
              pstream_shutdown(leader_qbot2_DW.StreamClient_Stream);
              pstream_shutdown(leader_qbot2_DW.StreamServer_Stream);
            }
