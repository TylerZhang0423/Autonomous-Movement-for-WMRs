...
typedef RT_MODEL_leader_qball2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * leader_qball2(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void leader_qball2_initialize();
   extern void leader_qball2_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(leader_qball2_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: leader_qball2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
        ...
        if (result == 0) {
          ...
          if (result == 0) {
            ...
            if (result == 0) {
              ...
              {
                pstream_shutdown(leader_qball2_DW.StreamClient_Stream);
                pstream_shutdown(leader_qball2_DW.StreamServer_Stream);
              }
