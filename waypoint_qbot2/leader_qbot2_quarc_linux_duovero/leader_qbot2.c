...
void leader_qbot2_step(void)
{
  ...
  /* RateLimiter: '<S3>/Rate Limiter - Turning Rate' */
  if (leader_qbot2_DW.LastMajorTime_l == (rtInf)) {
    leader_qbot2_B.RateLimiterTurningRate = rtb_Product;
  } else {
    currentTime = leader_qbot2_M->Timing.t[0] - leader_qbot2_DW.LastMajorTime_l;
    rtheta = currentTime * leader_qbot2_P.RateLimiterTurningRate_RisingLi;
    rtb_target_xy_idx_0 = rtb_Product - leader_qbot2_DW.PrevY_m;
    if (rtb_target_xy_idx_0 > rtheta) {
      leader_qbot2_B.RateLimiterTurningRate = leader_qbot2_DW.PrevY_m + rtheta;
    } else {
      currentTime *= leader_qbot2_P.RateLimiterTurningRate_FallingL;
      if (rtb_target_xy_idx_0 < currentTime) {
        leader_qbot2_B.RateLimiterTurningRate = leader_qbot2_DW.PrevY_m +
          currentTime;
      } else {
        leader_qbot2_B.RateLimiterTurningRate = rtb_Product;
      }
  ...
  /* End of RateLimiter: '<S3>/Rate Limiter - Turning Rate' */
  ...
  /* '<S19>:1:7' */
  leader_qbot2_B.vR = leader_qbot2_B.RateLimiterTurningRate * 0.235 / 2.0 +
    leader_qbot2_B.RateLimiterLinearSpeed;

  /* '<S19>:1:8' */
  leader_qbot2_B.vL = leader_qbot2_B.RateLimiterLinearSpeed -
    leader_qbot2_B.RateLimiterTurningRate * 0.235 / 2.0;
  if (rtmIsMajorTimeStep(leader_qbot2_M)) {
  ...
  if (rtmIsMajorTimeStep(leader_qbot2_M)) {
    ...
    /* Update for RateLimiter: '<S3>/Rate Limiter - Turning Rate' */
    leader_qbot2_DW.PrevY_m = leader_qbot2_B.RateLimiterTurningRate;
...
void leader_qbot2_initialize(void)
{
  ...
  /* External mode info */
  leader_qbot2_M->Sizes.checksums[0] = (2194751268U);
  leader_qbot2_M->Sizes.checksums[1] = (3817521069U);
  leader_qbot2_M->Sizes.checksums[2] = (1884771559U);
  leader_qbot2_M->Sizes.checksums[3] = (3659677574U);
  ...
  /* InitializeConditions for RateLimiter: '<S3>/Rate Limiter - Turning Rate' */
