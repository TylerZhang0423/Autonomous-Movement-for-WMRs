...
typedef RT_MODEL_tracking_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * tracking(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void tracking_initialize();
   extern void tracking_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(tracking_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: tracking -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
